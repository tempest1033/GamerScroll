...
| **본문 구조** | ☐ | 섹션당 3단락, 단락당 3문장(200자 내외), `\n\n`으로 구분 |
| **출처** | ☐ | 최소 4-5개 이상, 나무위키 제외 |
...
| 타입 | 용도 | 예시 |
|------|------|------|
| `text` | 본문 문단 | 3단락, 단락당 3문장(200자 내외), `\n\n`으로 문단 구분 |
...
| **언어** | 영문 소문자 권장, 한글도 허용 | `ddr5-ram-가격-폭등` |
...
| **alt** | 검색엔진용 | **키워드 포함 설명** (선택, 없으면 caption 사용) |
...
- **3단락 구조**: 첫 단락(도입) → 둘째 단락(전개) → 셋째 단락(마무리/전환)
- **늘어짐 방지**: 한 단락에 4문장 이상 금지, 200자 초과 시 단락 분리
...
| **heading** | 키워드로 시작, 마지막은 "마치며" | `Unity 엔진 특징`, `마치며` |
