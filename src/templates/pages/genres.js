...
function renderGenre(id = 'all') {
  ...
  const body = `<section class="section active" id="rankings">${generateHomeAdPairSlot(AD_SLOTS.PCHome001, AD_SLOTS.Mobile001)}<div class="page-container rk rk-genres"><div class="rk-head"><h1>${id === 'all' ? '장르별 모바일 게임 매출 순위' : `${label} 게임 매출 순위`}</h1></div>${subnav(S, 'genres')}${classification}<input type="radio" name="rk-store" id="rk-st-and" class="rk-control" checked><input type="radio" name="rk-store" id="rk-st-ios" class="rk-control"><div class="rk-storeseg"><label for="rk-st-and">구글플레이</label><label for="rk-st-ios">앱스토어</label></div><div class="rk-cols">${column('android', '구글플레이', 'and', android.length)}${column('ios', '앱스토어', 'ios', ios.length)}</div>${monthlyCard}</div></section>`;
