...
function renderGenre(id = 'all') {
  ...
  return wrapWithLayout(body, {
    currentPage: 'rankings', title: `${id === 'all' ? '장르별' : label} 모바일 게임 매출 순위 TOP ${Math.max(ios.length, android.length)} (${S.today.date}) | 게이머스크롤`,
    description: `${S.today.date} 한국 앱스토어·구글플레이 매출 TOP 200 중 ${label} 게임 ${Math.max(ios.length, android.length)}개의 순위${android[0] ? `. 구글플레이 1위 ${S.nameOf('android', android[0])}` : ''}${ios[0] ? `, 앱스토어 1위 ${S.nameOf('ios', ios[0])}` : ''}. ${S.latestMonth} 월간 TOP 10과 전체 매출 순위를 함께 제공합니다.`,
